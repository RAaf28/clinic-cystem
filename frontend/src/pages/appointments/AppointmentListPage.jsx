...
const AppointmentListPage = () => {
  ...
  const [showConfirm, setShowConfirm] = useState(null); // {id, action, label}
  ...
  const handleCreate = async (e) => {
    ...
    // Jika Pasien, patient_id diambil dari profileId
    const payload = {
      ...formData,
  ...
  const filtered = appointments.filter((a) => !statusFilter || a.status === statusFilter);
        ...
        <DataTable columns={COLUMNS} loading={isLoading} skeletonRows={5}>
          {filtered.length === 0 ? (
            <tr><td colSpan={COLUMNS.length}>
              <EmptyState icon="calendar_today" message="Belum ada janji temu." />
            </td></tr>
          ) : filtered.map((a) => (
              ...
              <td className="px-6 py-4">
                <div className="flex items-center gap-1">
                  <Link to={`/appointments/${a.id}`}>
                    <BtnIcon icon="visibility" title="Detail" />
                  </Link>
